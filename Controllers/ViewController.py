...
class ViewController(Controller):
    ...
    def UpdateDisplay(self):
        if self.ViewState:
            if self.ViewState.DrawFrame:
                Updates = self.ViewState.get_drawable_updates()
    ...
    def Notify(self, event):
        ...
        if isinstance(event, Events.UpdateViewEvent):
            ...
            self.ViewState.apply_updates(event.Updates)
        if isinstance(event, Events.ShiftOffsetEvent):
            self.ViewState.shift_offset(event.Delta)
